 ...
 */
add_filter('the_content','minimal_social_minimal_social_share_append_top', 10);
function minimal_social_minimal_social_share_append_top( $content ){
    if( get_theme_mod('social_share_append', 'none') == 'top' || get_theme_mod('social_share_append', 'none') == 'both' ){
        if( is_singular('post') && get_theme_mod('social_share_append_to', 'all') != 'pages' ){
            $social = minimall_get_social_media();
        }elseif( ( get_post_type() == 'page' && !is_page_template() ) && get_theme_mod('social_share_append_to', 'all') != 'posts' ){
            $social = minimall_get_social_media();
        }else{
            $social = '';
        }
 ...
 * Append custom share button to after content
 */
add_action('minimall_after_content','minimal_social_share_append_bottom', 40);
function minimal_social_share_append_bottom( ){
    if( get_theme_mod('social_share_append', 'none') == 'bottom' || get_theme_mod('social_share_append', 'none') == 'both' ){
        
        $social = '';
        $title = '';

        if( get_theme_mod('social_share_label', false) ){
            $title .= '<h5 class="mt2 mb2 mt0 hide-prin">' . esc_html( get_theme_mod('social_share_label', false) ) . '</h5>';
        }

        if( is_singular('post') && get_theme_mod('social_share_append_to', 'all') != 'pages' ){
            $social = $title . minimall_get_social_media();
        }elseif( ( get_post_type() == 'page' && !is_page_template() ) && get_theme_mod('social_share_append_to', 'all') != 'posts' ){
            $social = $title . minimall_get_social_media();
        }else{
            $social = '';
 ...
 */
function minimall_get_social_media(){
    $active_media = get_theme_mod('social_share', array('facebook', 'twitter', 'google', 'linkedin', 'pinterest'));
    
    $content = '';
    
    $content .= '<div class="social-share-btns hide-print max-width-3 ml-auto mr-auto mt2 mb2">';
